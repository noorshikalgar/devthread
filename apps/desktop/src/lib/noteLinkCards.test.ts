...
describe("findBareLinkInParagraph", () => {
  it("matches a single bare-URL paragraph", () => {
    const result = findBareLinkInParagraph(anchor("https://example.com/page", "https://example.com/page"));
  ...
  it("derives a host when the URL has no path", () => {
    const result = findBareLinkInParagraph(anchor("https://example.com", "https://example.com"));
    expect(result?.link.host).toBe("example.com");
  ...
  it("strips www. prefix from the host", () => {
    const result = findBareLinkInParagraph(anchor("https://www.example.com/post", "https://www.example.com/post"));
    expect(result?.link.host).toBe("example.com");
  });
  ...
  it("rejects a link whose text differs from the href", () => {
    const match = findBareLinkInParagraph(anchor("https://example.com", "click here"));
    expect(match).toBeNull();
  });
  ...
  it("returns null for non-http URLs", () => {
    const match = findBareLinkInParagraph(anchor("javascript:alert(1)", "javascript:alert(1)"));
    expect(match).toBeNull();
  });

  it("replaces dashes and underscores with spaces in the label", () => {
    const match = findBareLinkInParagraph(
      anchor("https://example.com/some_post-name", "https://example.com/some_post-name"),
    );
...
describe("paragraph shape helper", () => {
  it("returns a paragraph node containing the link", () => {
    const node = paragraph(anchor("https://example.com", "https://example.com"));
